...
def parse_args():
  ...
  parser.add_argument("srt_path", nargs="?", type=Path)
  ...
  missing = []
  if args.srt_path is None:
    missing.append("srt_path")
  if args.language is None:
    missing.append("-l/--language")
  if not args.list_models and missing:
...
def parse_input(input_path: Path):
  if input_path.is_dir():
    return parse_input_dir(input_path)
  return [input_path], [parse_input_file(input_path)]


def parse_input_dir(input_path: Path):
  files = sorted([f for f in input_path.iterdir() if f.suffix.casefold()==".srt"])
  parsed = [parse_input_file(f) for f in files]
  return files, parsed


def parse_input_file(input_path: Path):
...
def is_valid(results, original):
  if results is None or len(results) != len(original):
    print(f"Error: Length mismatched: {len(results) = }, {len(original) = }")
    return False

  for i, (r, o) in enumerate(zip(results, original)):
    if r['n'] != o['n']:
      print(f"Error: Index mismatched: {i}")
      return False
...
def translate(client, texts, nbatch, cooldown):
  ...
  for batch in tqdm(batched(texts, nbatch), total=math.ceil(len(texts)/nbatch)):
    batch_text = json.dumps(list(batch))
    try:
...
def main():
  ...
  if args.list_models:
    _ = client.list_models(display=True)
    return

  client.set_model(args.model)
  system_instruction = parse_instruction(args.language, args.example)

  files, srts = parse_input(args.srt_path)
  for fn, srt in zip(files, srts):
    print(str(fn))

    chat = client.create_chat(system_instruction, args.thinking)
    if chat is None:
    ...
    ltexts = len(texts)
    nbatch = math.ceil(ltexts / max(1, (math.ceil(ltexts/args.batch) - 1)))
    print(nbatch)

    success, failed = translate(chat, texts, nbatch, args.cooldown)
    while len(failed) > 0:
      chat = client.create_chat(system_instruction, args.thinking)
    ...
    write_srt(results, fn.with_suffix(f".{args.language.alpha_2}.srt"))
    if args.save_json:
      with fn.with_suffix(f".{args.language.alpha_2}.json").open("w") as f:
